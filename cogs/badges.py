...
class medals(commands.Cog):
    ...
    async def get_medals_data(self, type: str, username: str) -> str:
        async with aiohttp.ClientSession() as session:
            ...
            url1 = f"{BASE_URL}?q={query_string}"
            url2 = f"{BASE_URL}?q={query_string}&max=1000&skip=1000"

            async with session.get(url1, headers=HEADERS) as r1, session.get(url2, headers=HEADERS) as r2:
                data1 = await r1.json()
                data2 = await r2.json()
